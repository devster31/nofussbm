...
app.config.from_object('nofussbm.config.Config')
...
stderr_handler.setFormatter(Formatter('%(asctime)s [%(process)s] [%(levelname)s] [Flask: %(name)s] %(message)s',
                                      '%Y-%m-%d %H:%M:%S'))
    ...
    try:
        if list_appearance == 'html':
            for bm in list_query(email, bookmarks_per_page):
                ...
                result.append((date.strftime('%Y-%m-%d'), bm['url'],
                              bm['title'], bm['tags']))
            if content_only:
                return render_template('list-content.html', bookmarks=result)
            else:
                return render_template('list.html', bookmarks=result,
                                       top_tags=tags(mongo.db, email) if show_tags else None)
        else:
            for bm in list_query(email):
                date = bm['date-modified']
                result.append(u'\t'.join((date.strftime('%Y-%m-%d'), bm['url'],
                              bm['title'], u','.join(bm['tags']))))
            return textify(u'\n'.join(result))
    except OperationFailure:
        abort(500)
