...
def tags(db, email):
    try:
        ...
        tags_updated = None
    try:
        bookmarks_updated = db.bookmarks.find_one({'email': email}, {'date-modified': 1},
                                                  sort=[('date-modified', -1)])['date-modified']
    except TypeError:
        bookmarks_updated = None
    if not tags_updated or tags_updated < bookmarks_updated:
        _update_tags(db, email)
...
def _update_tags(db, email):
    ...
    db['tags-exapnded'].map_reduce(Code("""
        function() {
            emit({'email': this._id.email}, {'tags': [[this._id.tag, this.value.count]],
                 'modified': this.value.modified});
        }"""), Code("""
        function(key, values) {
